...
def test(data,
    ...
    if save_json and map50 and len(jdict):
        ...
        try:
            ...
            cocoGt = COCO(glob.glob('/work/handsomejw66/coco17/coco/annotations/instances_val*.json')[0])  # initialize COCO ground truth api
...
if __name__ == '__main__':
    ...
    if opt.task in ['val', 'test']:  # (default) run normally
        ...
        for weights in ['weights/cd53s-pacsp-s.pt', 'weights/cd53s-pacsp.pt', 'weights/cd53s-pacsp-x.pt']:
            f = 'full_%s_%s.txt' % (Path(opt.data).stem, Path(weights).stem)  # filename to save to
